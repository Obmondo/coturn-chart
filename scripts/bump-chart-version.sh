...
version=$(grep "^version:" "charts/coturn/Chart.yaml" | awk '{print $2}')
...
echo "Bumping version for coturn chart from $version to $major.$minor.$patch"
sed -i "s/^version:.*/version: ${major}.${minor}.${patch}/g" "charts/coturn/Chart.yaml"
